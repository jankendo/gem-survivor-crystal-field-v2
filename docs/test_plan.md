...
`workflow_dispatch full_test=true`は既存44本の長時間scriptを削除せず、通常群に加えてiOS perf 10/20/30分、energy、density 30/45/60分を個別化した13 shardで並列実行する。各shardは個別artifactを残し、1件でも失敗すればworkflowを失敗させる。
