...
タッチ倍速固定、手動終了時のリザルト清算、日本語の設定選択リスト、解放済み・ONだけのコア候補、解放前フィールド対象の完全非表示、実体に紐付くイベント誘導、詳細なショップ条件表示を追加しています。詳細は[docs/v2_phase8_ios_extreme_lite_gameplay_ux.md](docs/v2_phase8_ios_extreme_lite_gameplay_ux.md)を参照してください。
...
```powershell
& .tools/godot-4.7/editor/Godot_v4.7-stable_win64_console.exe --headless --path . --check-only
& .tools/godot-4.7/editor/Godot_v4.7-stable_win64_console.exe --headless --path . --script res://tests/smoke_main_scene.gd
& .tools/godot-4.7/editor/Godot_v4.7-stable_win64_console.exe --headless --path . --script res://tests/test_runner.gd
& .tools/godot-4.7/editor/Godot_v4.7-stable_win64_console.exe --headless --path . --script res://tests/phase8_test_runner.gd
& .tools/godot-4.7/editor/Godot_v4.7-stable_win64_console.exe --headless --path . --script res://tests/auto_play_ios_phase8_extreme_late_game_stress.gd
...
- Renderer: `gl_compatibility`（Windows/iOS共通）
- 言語: GDScript
- 主要データ: `data/*.json`
...
- [docs/performance/arena_render_cache_architecture.md](docs/performance/arena_render_cache_architecture.md)
- [docs/migration/godot_4_2_to_4_7.md](docs/migration/godot_4_2_to_4_7.md)
...
- Windows/headless計測は実iPhone、Metal、thermal、battery、実機60 FPSの証明ではない
