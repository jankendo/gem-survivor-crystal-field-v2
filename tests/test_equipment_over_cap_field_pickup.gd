...
func run(t) -> void:
	...
	t.assert_true(capacity.can_take(state, "weapon", "soul_scythe", true), "field/core pickup should be allowed to exceed cap")
	t.assert_true(grant.grant(state, "weapon", "soul_scythe", [], "field_equipment", true), "field grant should add a sixth weapon")
