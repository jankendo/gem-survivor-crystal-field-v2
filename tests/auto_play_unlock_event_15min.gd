...
func _run() -> void:
	...
	_assert((final_data.get("unlocked_weapons", []) as Array).size() > initial_weapons, "run should unlock weapons")
	_assert((final_data.get("unlocked_passives", []) as Array).size() > initial_passives, "run should unlock passives")
	_assert(_tree_has_text(main, "新武器解放"), "result should show weapon unlock notification")
	_assert(_tree_has_text(main, "新パッシブ解放"), "result should show passive unlock notification")
