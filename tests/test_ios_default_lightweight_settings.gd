...
func run(t) -> void:
	...
	t.assert_eq(int(settings.get("target_fps", 0)), 45, "iOS default should target 45 fps")
	...
	t.assert_eq(int(optimizer.budget.get("target_fps", 0)), 45, "battery saver budget should be selected by iOS defaults")
