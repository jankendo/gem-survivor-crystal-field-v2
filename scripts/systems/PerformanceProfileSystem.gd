...
func ui_limits(settings: Dictionary, platform: String = OS.get_name()) -> Dictionary:
	...
	return {
		"damage_numbers_enabled": bool(settings.get("damage_numbers", true)) and not minimal,
		"max_damage_numbers": 0 if minimal else (18 if ios and low else (28 if ios else 54)),
		...
		"max_rendered_damage_numbers": 0 if minimal else (24 if ios and low else (36 if ios else 80)),
