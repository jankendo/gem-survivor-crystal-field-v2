...
func setup(title: String, body: String, confirm_label: String = "決定", cancel_label: String = "戻る") -> void:
	...
	cancel_button.setup(cancel_label, Color(0.42, 0.82, 1.0), Vector2(140, 42))
	cancel_button.pressed.connect(func(): canceled.emit())
	row.add_child(cancel_button)
	var confirm_button = CrystalButtonScript.new()
	confirm_button.setup(confirm_label, Color(1.0, 0.32, 0.42), Vector2(140, 42), true)
