    ...
    OUT.write_text(
        "\n".join(
            [
                ...
                f"Enemy EXP drop multiplier: {settings.get('enemy_exp_drop_multiplier')}",
                f"Normal enemy EXP drop chance: {settings.get('normal_enemy_exp_drop_chance')}",
                ...
                "",
                "Invariant: required EXP depends only on level.",
                "Invariant: same enemy drop value, same drop cadence, and same gem value do not change by elapsed time.",
                "Formula: base gem value * normal balance multiplier * passive/event/rune modifiers * debug multiplier.",
